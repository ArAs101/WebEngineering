...
async function getImageOrPlaceholder(fileName: string | null): Promise<string> {
  ...
  try {
    ...
    console.warn(
      'Could not load image for ' + fileName + '. Using placeholder.',
      error
    );
    return PLACEHOLDER_IMAGE;
  }
...
async function extractBears(wikitext: string): Promise<Bear[]> {
  ...
  const bearPromises: Array<Promise<Bear>> = rows.map(
    async (row): Promise<Bear> => {
      const { imageFileName, ...bear } = parseBearRow(row);
      const image = await getImageOrPlaceholder(imageFileName);
      return {
        ...bear,
        image,
      };
